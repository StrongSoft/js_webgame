...
var 비동기콜백 = function(이벤트) { // 칸을 클릭했을 때
    ...
    const 몇줄 = 줄들.indexOf(이벤트.target.parentNode);
    const 몇칸 = 칸들[몇줄].indexOf(이벤트.target);
    ...
    if (칸들[몇줄][몇칸].textContent !== '') { // 칸이 이미 채워져 있는가?
        ...
        if (승리여부) { // 이겼을 경우
            ...
            setTimeout(function() {
                ...
                // 빈 칸 중 하나를 고른다.
                const 선택칸 = 후보칸[Math.floor(Math.random() * 후보칸.length)];
                선택칸.textContent = 턴;
                // 컴퓨터가 승리했는지 체크
                const 몇줄 = 줄들.indexOf(선택칸.parentNode);
                const 몇칸 = 칸들[몇줄].indexOf(선택칸);
                const 승리여부 = 결과체크(몇줄, 몇칸);
                // 다 찼으면
                if (승리여부) { // 컴퓨터가 이겼을 경우
                    초기화();
